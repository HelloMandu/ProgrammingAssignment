...
#define Rsize(root, end) end - root // 오른쪽 자식의 갯수를 구함
using namespace std;
...
int Find_inorder_root(vector<int> &arr, int root_index, int begin, int end) { // 중위 순회에서 루트의 index값을 반환
...
void Find_Postorder(int pre_root_index, int begin, int end) { // postorder을 찾음
	int ino_root_index = Find_inorder_root(pre, pre_root_index, begin, end); // 중위 순회의 루트인덱스를 찾음
	if (begin < end) {
		int L_size = Lsize(ino_root_index, begin), R_size = Rsize(ino_root_index, end); // 왼쪽사이즈와 오른쪽사이즈
		...
		if (R_size > 0) // 오른쪽자식이 있을시
			Find_Postorder(R_child, ino_root_index + 1, end); // R위치를 루트로
	}
	out << ino[ino_root_index] << '\n';
}

void Find_Preorder(int post_root_index, int begin, int end) { // preorder을 찾음
	int ino_root_index = Find_inorder_root(post, post_root_index, begin, end); // 중위 순회의 루트인덱스를 찾음
	out << ino[ino_root_index] << '\n';
	if (begin < end) {
		int L_size = Lsize(ino_root_index, begin), R_size = Rsize(ino_root_index, end); // 왼쪽사이즈와 오른쪽사이즈
		...
		if (R_size > 0) // 오른쪽자식이 있을시
			Find_Preorder(R_child, ino_root_index + 1, end); // R위치를 루트로
	}
}
